...
layui.define(["element","laytpl" ,"jquery"], function (exports) {
    ...
    var miniMenu = {
        ...
        /**
         * 将本地菜单数据转换为 layuimini 菜单格式
         * @param menuList 菜单数据数组
         * @param parentId 父级ID(用于生成唯一ID)
         */
        transformMenu: function (menuList, parentId) {
            menuList = menuList || [];
            var me = this;
            return me.each(menuList, function (idx, menu) {
                var id = (parentId === undefined || parentId === null) ? String(idx) : parentId + '-' + idx;
                var item = {
                    id: id,
                    title: menu.title || '',
                    icon: menu.icon || '',
                ...
                if (menu.child && menu.child.length) {
                    item.child = me.transformMenu(menu.child, id);
                }
                return item;
            });
        },
